...
export default function Example({doctor}) {
  ...
  useEffect(() => {
   ...
   const getdata = async () => {
     ...
     for (i; i < selectedDayMeetings?.length; i++) {
       const x = selectedDayMeetings[i]?.patient_id;
       const response = await axios.get(
         `https://mediline.vercel.app/api/patients_users?uid=${x}`
       );
...
useEffect(()=>{
    setsdm(list.filter((meeting) => isSameDay(parseISO(meeting.fnsdate), selectedDay)));
},[list,selectedDay])
            ...
            <ol className="mt-4 space-y-1 text-sm leading-6 text-gray-500">
              {selectedDayMeetings?.length > 0 ? (
                selectedDayMeetings.map((meeting,index) => (
                  <Meeting meeting={meeting} key={index} index={index} patient={appointmentlist} />
                ))
              ) : (
