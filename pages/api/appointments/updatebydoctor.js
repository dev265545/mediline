...
export default async function handler(req, res) {
  ...
  if (method === "GET") {
    try {
      const user = await Appointment.find({ patient_doctor_id: id });
      res.json({ status: 200, data: user });
    } catch (err) {
      res.status(500).json(err);
    }
  ...
  if (method === "POST") {
    try {
      ...
      const order = await Appointment.findOneAndUpdate(
...
//   }
  ...
  // }
    if (method === "DELETE") {
Appointment.deleteOne({ "patient_doctor_id": id }, function (err) {
  if (err) {
    res.send(err);
  } else {
    res.send("deleted");
